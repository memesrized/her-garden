...
def main() -> None:
    ...
    notebook.cells = [
        nbformat.v4.new_markdown_cell(
            ...
            "Archive and restore now hide mistaken entities without deleting their histories. "
            "This uses the existing per-entity projection because household-scale data does not need "
            "snapshots or a background worker. Physical deletion remains deliberately unsupported. "
        ...
        nbformat.v4.new_code_cell(
            ...
            "'(unknown, not zero)')\n"
            "        await store.append_plant_event(uuid4(), plant_id, PlantEvent(\n"
            "            event_type='archive', occurred_at=now, note='Temporarily hide mistaken entry'))\n"
            "        print('Archived plant hidden from normal list:', "
            "len(await store.list_entities('plant')) == 0)\n"
            "        print('Archived plant remains recoverable:', "
            "(await store.get_plant_context(plant_id))['state']['archived'])\n"
            "        await store.append_plant_event(uuid4(), plant_id, PlantEvent(\n"
            "            event_type='restore', occurred_at=now, note='Entry confirmed'))\n"
            "        print('Restored plant visible again:', "
            "len(await store.list_entities('plant')) == 1)\n"
            "    finally:\n"
            "        await store.close()\n"
            "        async with await psycopg.AsyncConnection.connect(dsn, autocommit=True) as conn:\n"
